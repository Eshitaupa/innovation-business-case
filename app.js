...
const CONFIG = {
  ...
  // localStorage key — persists across browser refreshes
...
// =============================================================================
// CACHE  (localStorage — survives page/browser refresh)
// =============================================================================
function saveCache(data) {
  try {
    localStorage.setItem(CONFIG.cacheKey, JSON.stringify({ ts: Date.now(), data }));
...
async function reloadRecords() {
  ...
  await fetchAndUpdateSilently({ toast: true });
}
...
function clearAllEditors() {
  CONFIG.richTextFields.forEach(fieldName => setEditorContent(fieldName, ""));
  state.richTextValues = {};
}
...
function buildPeopleSelect() {
  ...
  filterInput.addEventListener("input", () => {
    ...
    const matches = state.allUsers
      .filter(u => (u.Title||"").toLowerCase().includes(q) || (u.Email||"").toLowerCase().includes(q))
      .slice(0, 50);
    while (sel.firstChild) sel.removeChild(sel.firstChild);
    if (!matches.length) {
      const empty = document.createElement("option");
      ...
      if (state.selectedPerson?.claims) {
        for (let i = 0; i < sel.options.length; i++) {
          if (sel.options[i].value === state.selectedPerson.claims) { sel.options[i].selected = true; break; }
        }
      }
    }
    sel.style.display = "block";
  ...
  sel.addEventListener("change", () => {
    ...
    if (!opt || opt.disabled) return;
    state.selectedPerson = {
      displayName: opt.getAttribute("data-name")  || "",
      email:       opt.getAttribute("data-email") || "",
      claims:      opt.value
    };
    const hiddenField = els.caseForm?.elements.personClaims;
    if (hiddenField) hiddenField.value = opt.value;
    filterInput.value = state.selectedPerson.displayName || state.selectedPerson.email;
    sel.style.display = "none";
    while (sel.firstChild) sel.removeChild(sel.firstChild);
  });
}
...
function fillPersonPicker(record) {
  if (!record.personDisplayName && !record.personEmail) return;
  state.selectedPerson = { displayName: record.personDisplayName, email: record.personEmail, claims: record.personClaims };
  const filterInput = document.getElementById("personFilterInput");
  const sel         = document.getElementById("personSelect");
  if (!filterInput || !sel) return;
  filterInput.value = record.personDisplayName || record.personEmail;
...
function resetPersonPicker() {
  ...
  const sel         = document.getElementById("personSelect");
  if (filterInput) filterInput.value = "";
  if (sel) { while (sel.firstChild) sel.removeChild(sel.firstChild); sel.style.display = "none"; }
  const hiddenField = els.caseForm?.elements.personClaims;
  if (hiddenField) hiddenField.value = "";
}
...
function handleDeleteClick(id) {
  ...
  const rec  = state.records.find(r => String(r.id) === String(id));
  const name = rec?.ideaName || "this record";
  showConfirmModal("Delete business case",
    `Are you sure you want to delete "<strong>${esc(name)}</strong>"? This action cannot be undone.`,
    "Delete",
...
async function deleteRecord(id) {
  ...
  try {
    ...
    const text = await res.text().catch(() => "");
    if (!res.ok) throw new Error(`HTTP ${res.status}: ${text || "Flow returned an error"}`);
    clearCache();
    showToast("✓ Record deleted.");
    fetchAndUpdateSilently();
  } catch (err) {
...
function showConfirmModal(title, bodyHtml, confirmLabel, onConfirm) {
  ...
  Object.assign(modal.style, {
    ...
    background:"rgba(18,26,22,0.5)", display:"flex",
    alignItems:"center", justifyContent:"center", padding:"20px"
  });
  const name = bodyHtml.replace(/<[^>]*>/g, "").replace(/"/g, "");
  modal.innerHTML = `
    <div style="background:var(--panel,#fff);border-radius:12px;border:1px solid var(--danger-line,#fca5a5);
      box-shadow:0 18px 48px rgba(13,32,66,0.18);padding:24px;max-width:440px;width:100%;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
        <div style="width:36px;height:36px;border-radius:50%;background:var(--danger-soft,#fff1f0);
          display:flex;align-items:center;justify-content:center;flex:0 0 auto;">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--danger,#b42318)"
        ...
        <h3 id="confirmModalTitle" style="margin:0;flex:1;font-size:17px;font-weight:700;color:var(--ink,#0f1c2e);">
          ${esc(title)}</h3>
        <button id="confirmClose" type="button" aria-label="Close"
          style="background:transparent;border:none;cursor:pointer;padding:4px;color:var(--muted,#6b7c93);display:flex;align-items:center;">
      ...
      <p style="margin:0 0 6px;font-size:14px;line-height:1.6;color:var(--ink,#0f1c2e);">Are you sure you want to delete</p>
      <p style="margin:0 0 8px;font-size:14px;font-weight:700;color:var(--ink,#0f1c2e);">"${esc(name)}"</p>
      <p style="margin:0 0 20px;font-size:13px;color:var(--muted,#6b7c93);">This action cannot be undone.</p>
      <div style="display:flex;gap:10px;justify-content:flex-end;">
        <button id="confirmCancel" type="button"
          style="min-height:38px;padding:0 16px;border:1px solid var(--line,#d6e0f0);border-radius:8px;
            background:var(--panel,#fff);color:var(--ink,#0f1c2e);font:inherit;font-weight:650;cursor:pointer;">
          Cancel</button>
        <button id="confirmOk" type="button"
          style="min-height:38px;padding:0 16px;border:1px solid var(--danger,#b42318);border-radius:8px;
            background:var(--danger,#b42318);color:#fff;font:inherit;font-weight:650;cursor:pointer;
            display:inline-flex;align-items:center;gap:8px;">
  ...
  modal.querySelector("#confirmClose").addEventListener("click", close);
  modal.querySelector("#confirmCancel").addEventListener("click", close);
  modal.querySelector("#confirmOk").addEventListener("click", () => { close(); onConfirm(); });
...
function renderTable() {
  ...
  const rows = filtered();
  if (!rows.length) {
    els.caseRows.innerHTML = `<tr class="empty-row"><td colspan="10">No business cases match the current view.</td></tr>`;
    return;
  }
  els.caseRows.innerHTML = rows.map(r => `
    <tr${r._isTemp ? ' class="row-temp"' : ''}>
      <td class="idea-cell">${esc(r.ideaName)}${r._isTemp ? ' <span class="saving-badge">saving…</span>' : ''}</td>
      <td>${esc(r.personDisplayName)}</td>
      <td class="text-cell">${stripHtml(r.problemStatement)}</td>
      <td class="number-cell">${fmtPlain(r.costSavings)}</td>
      <td class="number-cell">${fmtPct(r.efficiencyGain)}</td>
      ...
      <td class="action-col">
        <div class="row-actions">
          <button class="icon-button row-action" type="button"
            title="Edit" aria-label="Edit ${escAttr(r.ideaName || "case")}"
            data-edit-id="${escAttr(r.id)}"${r._isTemp ? ' disabled' : ''}>
            <svg><use href="#icon-edit"></use></svg>
          </button>
          <button class="icon-button row-action row-delete" type="button"
            title="Delete" aria-label="Delete ${escAttr(r.ideaName || "case")}"
            data-delete-id="${escAttr(r.id)}"${r._isTemp ? ' disabled' : ''}>
            <svg><use href="#icon-trash"></use></svg>
          </button>
        </div>
      </td>
  ...
  els.caseRows.querySelectorAll("[data-edit-id]").forEach(btn =>
    btn.addEventListener("click", () => {
      const rec = state.records.find(x => String(x.id) === String(btn.dataset.editId));
      if (rec && !rec._isTemp) openDrawer(rec);
    })
  );
  els.caseRows.querySelectorAll("[data-delete-id]").forEach(btn =>
    btn.addEventListener("click", () => {
      const rec = state.records.find(x => String(x.id) === String(btn.dataset.deleteId));
      if (rec && !rec._isTemp) handleDeleteClick(btn.dataset.deleteId);
    })
  );
}
...
function filtered() {
  ...
  return [...state.records]
    ...
    .sort((a, b) => {
      // Temp (optimistic) records always sort to top
      if (a._isTemp && !b._isTemp) return -1;
      if (!a._isTemp && b._isTemp) return 1;
...
function openDrawer(record = null) {
  ...
  if (record) {
    for (const key of Object.keys(CONFIG.fieldMap)) {
      ...
      const v = record[key];
      if (v == null) continue;
      if (CONFIG.richTextFields.has(key)) { setEditorContent(key, v); } else { ctrl.value = v; }
    }
    els.caseForm.elements.id.value = record.id;
    fillPersonPicker(record);
  } else {
  ...
  errors.forEach(({ field, message }) => {
    ...
    if (CONFIG.richTextFields.has(field)) {
      ...
      if (editor) {
        editor.classList.add("field-error");
        const wrapper = editor.closest(".rich-editor-wrapper");
        if (wrapper && !wrapper.querySelector(".field-error-msg")) {
          const errSpan = document.createElement("span");
          errSpan.className = "field-error-msg";
          errSpan.textContent = message;
          wrapper.appendChild(errSpan);
...
// =============================================================================
// SAVE  — optimistic update, no blocking post-save fetch
// =============================================================================
async function saveCurrentCase(e) {
  e.preventDefault();
  const fd     = new FormData(els.caseForm);
  const record = formToRecord(fd);
  const errors = validateForm(record);
  if (errors.length) { showFieldErrors(errors); showErrorModal(errors); return; }
  ...
  if (isUpdate) {
    // Patch the existing record in place immediately
    ...
    // Insert a temporary record so the user sees it straight away
    ...
    state.records.unshift({
      ...
      _isTemp:  true   // flag used by renderTable to show "saving…" badge
    });
  }
...
function parseSaveError(errMsg) {
  ...
  [
    ...
    { pattern: /field_14|payback/i,  field: "paybackMonths", label: "Payback period months" },
    { pattern: /field_16|adoption/i, field: "adoptionRate",  label: "Adoption rate %" }
  ].forEach(({ pattern, field, label }) => {
    if (pattern.test(errMsg)) errors.push({ field, message: `${label}: ${errMsg}` });
  });
  return errors;
}
...
function exportCsv() {
  const cols = [
    ...
    ["scalabilityNotes","Scalable to all GPs"],["confidenceLevel","Confidence Level"],
    ["modified","Updated"]
  ];
  // Exclude temp (unsaved) records from export
  const rows = filtered().filter(r => !r._isTemp);
  const csv  = [
    cols.map(([,l]) => csvQ(l)).join(","),
    ...rows.map(r => cols.map(([k]) => csvQ(CONFIG.richTextFields.has(k) ? stripHtml(r[k]) : r[k])).join(","))
  ].join("\r\n");
  const a = Object.assign(document.createElement("a"), {
    href:     URL.createObjectURL(new Blob([csv], { type: "text/csv;charset=utf-8" })),
    download: `innovation-cases-${new Date().toISOString().slice(0,10)}.csv`
...
function numOrZero(v) { return (v===""||v==null||isNaN(Number(v)))?0:Number(v); }
...
function fmtPlain(v) {
  ...
  return new Intl.NumberFormat("en-US",{maximumFractionDigits:n%1!==0?2:0}).format(n);
}
...
function fmtN(v,d=0) {
  const n=Number(v)||0;
  return new Intl.NumberFormat("en-US",{maximumFractionDigits:d,minimumFractionDigits:d>0&&n%1!==0?d:0}).format(n);
}
