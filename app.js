...
deleteFlowUrl: "https://defaultbfbb9a2b6d994e78b3c795005d555c.8b.environment.api.powerplatform.com:443/powerautomate/automations/direct/workflows/b78b3db90e304888a38d3f594c1932dd/triggers/manual/paths/invoke?api-version=1&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=dl0m779qsg2IlSRUa_-S1paDAbW3uNjL8kKzlo4csZQ",
  ...
  // Cache key and max age in milliseconds (2 minutes)
  cacheKey: "ogc_innovation_cache",
...
function loadCache() {
  try {
    ...
    if (Date.now() - ts > CONFIG.cacheMaxAge) return null;
    return data;
  } catch { return null; }
}
...
// =============================================================================
// DATA LOAD  — show cache instantly, then fetch fresh in background
// =============================================================================
async function loadFromFlow() {
  ...
  if (cached) {
    ...
    render();
    // Fetch fresh data silently in background (no busy spinner)
    fetchAndUpdateSilently();
    return;
  }

  // 2. No cache — must wait for network (show busy)
  setBusy(true);
  try {
...
// Silent background refresh — no spinner, no blocking
async function fetchAndUpdateSilently() {
  try {
    const data = await fetchFlowData();
    applyFlowData(data);
    saveCache(data);
    ...
    // Don't show a toast — stale data is still usable
  }
}

async function reloadRecords() {
  // Show stale data instantly from cache
  const cached = loadCache();
  if (cached) {
    applyFlowData(cached);
    render();
  ...
  // Then refresh silently — show a subtle indicator via badge
